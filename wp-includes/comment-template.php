...
function comment_form_title( $noreplytext = false, $replytext = false, $linktoparent = true ) {
	...
	if ( false === $noreplytext ) $noreplytext = __( 'Написать комментарий' );
	if ( false === $replytext ) $replytext = __( 'Написать комментарий %s' );
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Имя' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
	...
	$required_text = sprintf( ' ' . __('Обязательные поля отмечены %s'), '<span class="required">*</span>' );
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Отзыв', 'noun' ) . '</label> <textarea id="comment" name="comment" cols="45" rows="8"  aria-required="true" required="required"></textarea></p>',
		...
		'comment_notes_before' => '<p class="comment-notes"><span id="email-notes">' . __( 'Ваш email не будет опубликован.' ) . '</span>'. ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		...
		'title_reply_to'       => __( 'Написать комментарий %s' ),
